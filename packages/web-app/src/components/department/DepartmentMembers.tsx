...
interface DepartmentMembersProps {
  isLoadingDepartmentUsers: boolean;
  departmentUsersError: boolean;
  users: User[];
  handleModalClose: () => void;
  handleDeleteMember: (id: number) => void;
}

const DepartmentMembers: React.FC<DepartmentMembersProps> = ({
      ...
      <div className='bg-white p-6 rounded-lg shadow-lg w-96'>
        ...
        <div className='space-y-4 max-h-64 overflow-y-auto'>
          {isLoadingDepartmentUsers ? (
            ...
            <p className='text-red-600'>Failed to load department members.</p>
          ) : users.length > 0 ? (
            users.map((user) => (
              <div
